...
package com.ictech.cupidlove.utils.purchase.IInAppBillingService.aidl.util;
...
// This is the key line that fixed everything for me
    ...
    public String getPricesDev() throws RemoteException, JSONException {
        ...
        ArrayList<String> skuList = new ArrayList<String>();
        if (Constant.TESTSKU != null && !Constant.TESTSKU.equals("")) {
            skuList.add(Constant.TESTSKU);
        }

        if (Constant.SKULOCATION != null && !Constant.SKULOCATION.equals("")) {
            skuList.add(Constant.SKULOCATION);
        }

        if (Constant.SKUREMOVEAD != null && !Constant.SKUREMOVEAD.equals("")) {
            skuList.add(Constant.SKUREMOVEAD);
        }

        if (Constant.SKUSPERLIKE != null && !Constant.SKUSPERLIKE.equals("")) {
            skuList.add(Constant.SKUSPERLIKE);
        }

        if (Constant.SKUCHAT != null && !Constant.SKUCHAT.equals("")) {
            skuList.add(Constant.SKUCHAT);
        }

        Bundle querySkus = new Bundle();
        querySkus.putStringArrayList("ITEM_ID_LIST", skuList);

        Bundle skuDetails = mService.getSkuDetails(3, Constant.PACKAGE_NAME, "inapp", querySkus);
        ...
        if (response == 0) {
            ...
            for (String thisResponse : responseList) {
                ...
                if (sku.equals(Constant.SKUREMOVEAD) && Prefs.getBoolean(RequestParamUtils.ADENABLED, false)) {

                } else if (sku.equals(Constant.SKUSPERLIKE) && Prefs.getBoolean(RequestParamUtils.ALLOWSUPERLIKEINAPPPURCHASE, false)) {

                } else if (sku.equals(Constant.SKULOCATION) && Prefs.getBoolean(RequestParamUtils.ALLOWLOCATIONINAPPPURCHASE, false)) {

                } else if (sku.equals(Constant.SKUCHAT) && Prefs.getBoolean(RequestParamUtils.ALLOWCHATINAPPPURCHASE, false)) {

                } else {
                    InAppPurchase.getList().add(new InAppPurchase(sku, price, name, description));
                }
